...
allowed_effects.add(state.State)
...
def _hoist_consts_to_refs(jaxpr: core.Jaxpr) -> core.Jaxpr:
  ...
  const_avals = [state.AbstractRef(var.aval.shape, var.aval.dtype,
...
def val_to_ref_aval(x) -> state.AbstractRef:
  aval = core.raise_to_shaped(core.get_aval(x))
  if type(aval) is not core.ShapedArray:
    raise Exception(f"can't make ref from {x}")
  return state.AbstractRef(aval.shape, aval.dtype, aval.weak_type)
