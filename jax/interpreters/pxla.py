...
def lower_parallel_callable(
  ...
  with maybe_extend_axis_env(axis_name, global_axis_size, None):  # type: ignore
    effects = [eff for eff in closed_jaxpr.effects
...
class ExecuteReplicated:
  ...
  def __init__(self, xla_executable, backend, in_handler: InputsHandler,
               out_handler: ResultsHandler, effects: List[core.Effect],
               keepalive: Any):
    self.xla_executable = xla_executable
  ...
  def __call__(self, *args):
    ...
    # if self.effects:
    #   input_bufs_flat, token_handler = dispatch._add_tokens(effects, device, input_bufs_flat)
...
def lower_mesh_computation(
  ...
  return MeshComputation(
      ...
      auto_spmd_lowering=auto_spmd_lowering, keepalive=keepalive)
